...
describe('async poll externalId contract', () => {
  ...
  it('parses and formats SUB2API image externalId', () => {
    const externalId = formatExternalId('SUB2API', 'IMAGE', 'img_req_123')
    expect(externalId).toBe('SUB2API:IMAGE:img_req_123')

    const parsed = parseExternalId(externalId)
    expect(parsed.provider).toBe('SUB2API')
    expect(parsed.type).toBe('IMAGE')
    expect(parsed.requestId).toBe('img_req_123')
  })

  it('parses and formats third-party Veo video externalId', () => {
    const externalId = formatExternalId('THIRDPARTY_VEO', 'VIDEO', 'task_123')
    expect(externalId).toBe('THIRDPARTY_VEO:VIDEO:task_123')

    const parsed = parseExternalId(externalId)
    expect(parsed.provider).toBe('THIRDPARTY_VEO')
    expect(parsed.type).toBe('VIDEO')
    expect(parsed.requestId).toBe('task_123')
  })
})
