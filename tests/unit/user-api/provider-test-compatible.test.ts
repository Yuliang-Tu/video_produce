...
describe('provider test connection compatible probes', () => {
  ...
  it('supports sub2api through compatible provider probes', async () => {
    fetchMock.mockImplementation(async (input: RequestInfo | URL) => {
      const url = String(input)
      if (url.endsWith('/v1/models')) {
        return new Response(JSON.stringify({ data: [{ id: 'gpt-5.5' }, { id: 'gpt-image-2' }] }), { status: 200 })
      }
      return new Response('not-found', { status: 404 })
    })
    ...
    const result = await testProviderConnection({
      apiType: 'sub2api',
      baseUrl: 'http://121.127.253.220:8080',
      apiKey: 'sub2api-key',
    })
  ...
  it('uses default local base url for sub2api when baseUrl is missing', async () => {
    fetchMock.mockImplementation(async (input: RequestInfo | URL) => {
      const url = String(input)
      if (url === 'http://121.127.253.220:8080/v1/models') {
  ...
  it('supports xingtuo llm through compatible provider probes when llm model is provided', async () => {
    fetchMock.mockImplementation(async (input: RequestInfo | URL) => {
      const url = String(input)
      if (url === 'http://121.127.253.220:8090/v1/models') {
        return new Response(JSON.stringify({ data: [{ id: 'gpt-5.5' }] }), { status: 200 })
      }
      return new Response('not-found', { status: 404 })
    })
