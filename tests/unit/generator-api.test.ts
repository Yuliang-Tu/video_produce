...
describe('generator-api gateway routing', () => {
  ...
  it('routes xingtuo image to dedicated generator', async () => {
    resolveModelSelectionMock.mockResolvedValueOnce({
      provider: 'xingtuo',
      modelId: 'gpt-image-2',
      modelKey: 'xingtuo::gpt-image-2',
      mediaType: 'image',
    })
    getProviderConfigMock.mockResolvedValueOnce({
      id: 'xingtuo',
    ...
    const result = await generateImage('user-1', 'xingtuo::gpt-image-2', 'draw cat', {
      aspectRatio: '1:1',
    })

    expect(createImageGeneratorMock).toHaveBeenCalledWith('xingtuo', 'gpt-image-2')
    expect(generateImageViaOpenAICompatMock).not.toHaveBeenCalled()
  ...
  it('routes thirdparty-veo video requests to dedicated provider generator', async () => {
    resolveModelSelectionMock.mockResolvedValueOnce({
      provider: 'thirdparty-veo',
      modelId: 'veo_3_1-fast',
      modelKey: 'thirdparty-veo::veo_3_1-fast',
      mediaType: 'video',
    })
    resolveModelGatewayRouteMock.mockReturnValueOnce('official')

    const result = await generateVideo('user-1', 'thirdparty-veo::veo_3_1-fast', 'https://example.com/source.png', {
      prompt: 'animate',
    })

    expect(createVideoGeneratorMock).toHaveBeenCalledWith('thirdparty-veo')
    expect(generateVideoViaOpenAICompatMock).not.toHaveBeenCalled()
