...
describe('api specific - user api-config PUT provider uniqueness', () => {
  ...
  it('pins sub2api provider baseUrl to local default when baseUrl is omitted', async () => {
    installAuthMocks()
    mockAuthenticated('user-1')
    const route = await import('@/app/api/user/api-config/route')
    ...
    const req = buildMockRequest({
      ...
      body: {
        providers: [
          { id: 'sub2api', name: 'Sub2API', apiKey: 'sub2api-key' },
        ],
      },
    })
    ...
    expect(savedProviders[0]).toMatchObject({
      id: 'sub2api',
      baseUrl: 'http://121.127.253.220:8080/v1',
      gatewayRoute: 'openai-compat',
    })
  })

  it('normalizes stored xingtuo legacy official gatewayRoute on GET', async () => {
    installAuthMocks()
    mockAuthenticated('user-1')
    const route = await import('@/app/api/user/api-config/route')
      ...
      customProviders: JSON.stringify([
        {
          id: 'xingtuo',
          name: 'Xingtuo',
          baseUrl: 'http://121.127.253.220:8090/v1',
          apiKey: 'enc:xingtuo-key',
    ...
    expect(res.status).toBe(200)
    const payload = await res.json() as { providers?: SavedProvider[] }
  ...
  it('normalizes sub2api provider baseUrl to include v1', async () => {
    installAuthMocks()
    mockAuthenticated('user-1')
    const route = await import('@/app/api/user/api-config/route')
    ...
    const req = buildMockRequest({
      ...
      body: {
        providers: [
          { id: 'sub2api', name: 'Sub2API', baseUrl: 'http://121.127.253.220:8080', apiKey: 'sub2api-key' },
        ],
      },
    })
  ...
  it('normalizes thirdparty-veo provider baseUrl to include v1', async () => {
    installAuthMocks()
    mockAuthenticated('user-1')
    const route = await import('@/app/api/user/api-config/route')
    ...
    const req = buildMockRequest({
      ...
      body: {
        providers: [
          { id: 'thirdparty-veo', name: 'Third-party Veo', baseUrl: 'https://veo.test', apiKey: 'veo-key' },
        ],
      },
    })
