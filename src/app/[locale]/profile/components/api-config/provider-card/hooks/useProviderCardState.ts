...
export function shouldProbeModelLlmProtocol(params: {
  ...
  return (providerKey === 'openai-compatible' || providerKey === 'xingtuo') && params.modelType === 'llm'
}
...
export function shouldReprobeModelLlmProtocol(params: {
  ...
  const originalProviderKey = getProviderKey(params.originalModel.provider)
  if (originalProviderKey !== 'openai-compatible' && originalProviderKey !== 'xingtuo') return false
...
export function buildProviderConnectionPayload(params: {
  ...
  const isCompatibleProvider =
    params.providerKey === 'openai-compatible' || params.providerKey === 'gemini-compatible' || params.providerKey === 'xingtuo'
...
export function useProviderCardState({
  ...
  const showBaseUrlEdit =
    ['gemini-compatible', 'openai-compatible', 'xingtuo'].includes(providerKey) &&
