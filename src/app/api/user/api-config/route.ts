...
const SUB2API_DEFAULT_BASE_URL = 'http://121.127.253.220:8080'
const XINGTUO_DEFAULT_BASE_URL = 'http://121.127.253.220:8090/v1'
...
function normalizeVersionedBaseUrl(baseUrl: string): string {
  try {
...
function normalizeProviderBaseUrl(input: {
  providerId: string
  baseUrl?: string
  strict: boolean
  field: string
  ...
  if (providerKey === 'sub2api' || providerKey === 'xingtuo') {
    return normalizeVersionedBaseUrl(input.baseUrl || (providerKey === 'xingtuo' ? XINGTUO_DEFAULT_BASE_URL : SUB2API_DEFAULT_BASE_URL))
  }
  if (providerKey === 'thirdparty-veo') {
    return input.baseUrl ? normalizeVersionedBaseUrl(input.baseUrl) : undefined
  }
...
function resolveProviderGatewayRoute(
  ...
  options?: { allowStoredLegacyOfficialCompat?: boolean },
): GatewayRouteType {
  const providerKey = getProviderKey(providerId)
  const isOpenAICompatibleProvider = providerKey === 'openai-compatible' || providerKey === 'sub2api' || providerKey === 'xingtuo'
  ...
  if (isOpenAICompatibleProvider) {
    if (rawGatewayRoute === 'official') {
      if (options?.allowStoredLegacyOfficialCompat) {
...
function isOpenAICompatibleLlmModel(model: StoredModel): boolean {
  ...
  return model.type === 'llm' && (providerKey === 'openai-compatible' || providerKey === 'sub2api' || providerKey === 'xingtuo')
}
  ...
  for (let index = 0; index < parsedUnknown.length; index += 1) {
    ...
    try {
      gatewayRoute = resolveProviderGatewayRoute(id, raw.gatewayRoute, {
        allowStoredLegacyOfficialCompat: true,
      })
    } catch {
      throw new ApiError('INVALID_PARAMS', {
        code: 'PROVIDER_GATEWAY_ROUTE_INVALID',
