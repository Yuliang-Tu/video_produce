...
function normalizeProviderBaseUrl(providerId: string, rawBaseUrl?: string): string | undefined {
  ...
  const baseUrl = readTrimmedString(rawBaseUrl)
    || (providerKey === 'sub2api' ? 'http://121.127.253.220:8080' : '')
    || (providerKey === 'xingtuo' ? 'http://121.127.253.220:8090/v1' : '')
  if (!baseUrl) return undefined
  if (
    providerKey !== 'openai-compatible'
    && providerKey !== 'sub2api'
    && providerKey !== 'xingtuo'
    && providerKey !== 'thirdparty-veo'
  ) return baseUrl
...
function parseCustomProviders(rawProviders: string | null | undefined): CustomProvider[] {
  ...
  for (let index = 0; index < parsedUnknown.length; index += 1) {
    ...
    } else if ((providerKey === 'openai-compatible' || providerKey === 'sub2api' || providerKey === 'xingtuo') && gatewayRouteRaw === 'official') {
      throw new Error(`PROVIDER_GATEWAY_ROUTE_INVALID: providers[${index}].gatewayRoute`)
    } else if (providerKey !== 'openai-compatible' && providerKey !== 'sub2api' && providerKey !== 'xingtuo' && gatewayRouteRaw === 'openai-compat') {
      throw new Error(`PROVIDER_GATEWAY_ROUTE_INVALID: providers[${index}].gatewayRoute`)
    } else {
...
export async function resolveModelSelection(
  ...
  const llmProtocol = mediaType === 'llm' && (providerKey === 'openai-compatible' || providerKey === 'sub2api' || providerKey === 'xingtuo')
