...
import { getXingtuoImageApiKey, getXingtuoImageBaseUrl } from './xingtuo-image-config'
...
const THIRDPARTY_VEO_PROVIDER_ID = 'thirdparty-veo'
...
export function parseExternalId(externalId: string): {
    provider: 'FAL' | 'ARK' | 'GEMINI' | 'GOOGLE' | 'MINIMAX' | 'VIDU' | 'OPENAI' | 'OCOMPAT' | 'BAILIAN' | 'SILICONFLOW' | 'SUB2API' | 'XINGTUO' | 'THIRDPARTY_VEO' | 'UNKNOWN'
    ...
    if (externalId.startsWith('SUB2API:')) {
        const parts = externalId.split(':')
        const type = parts[1]
        const requestId = parts.slice(2).join(':')
        if (type !== 'IMAGE' || !requestId) {
            throw new Error(`无效 SUB2API externalId: "${externalId}"，应为 SUB2API:IMAGE:requestId`)
        }
        return {
            provider: 'SUB2API',
            type: 'IMAGE',
    ...
    if (externalId.startsWith('XINGTUO:')) {
        const parts = externalId.split(':')
        const type = parts[1]
        const requestId = parts.slice(2).join(':')
        if (type !== 'IMAGE' || !requestId) {
            throw new Error(`无效 XINGTUO externalId: "${externalId}"，应为 XINGTUO:IMAGE:requestId`)
        }
        return {
            provider: 'XINGTUO',
            type: 'IMAGE',
    ...
    if (externalId.startsWith('THIRDPARTY_VEO:')) {
        const parts = externalId.split(':')
        const type = parts[1]
        const requestId = parts.slice(2).join(':')
        if (type !== 'VIDEO' || !requestId) {
            throw new Error(`Invalid THIRDPARTY_VEO externalId: "${externalId}", expected THIRDPARTY_VEO:VIDEO:taskId`)
        }
        return {
            provider: 'THIRDPARTY_VEO',
            type: 'VIDEO',
            requestId,
        }
    }
...
export async function pollAsyncTask(
    ...
    switch (parsed.provider) {
        ...
        case 'SUB2API':
        case 'XINGTUO':
            return await pollXingtuoImageTask(parsed.requestId, userId)
        case 'THIRDPARTY_VEO':
            return await pollThirdpartyVeoTask(parsed.requestId, userId)
        default:
...
function getFirstSub2ApiImageUrl(payload: unknown): string {
...
async function pollXingtuoImageTask(requestId: string, userId: string): Promise<PollResult> {
    const config = await getProviderConfig(userId, 'xingtuo')
    const baseUrl = getXingtuoImageBaseUrl(config.baseUrl)
    const apiKey = getXingtuoImageApiKey(config.apiKey)
    const response = await fetch(`${baseUrl}/xingtuo/image2/jobs/${encodeURIComponent(requestId)}`, {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
        return {
            status: 'failed',
            error: `Xingtuo image status request failed: ${response.status} ${rawText.slice(0, 200)}`,
        }
    }

    const statusRaw = payload && typeof payload === 'object'
        ? (payload as { status?: unknown }).status
        : ''
    const status = typeof statusRaw === 'string' ? statusRaw.trim().toLowerCase() : ''

    if (status === 'success' || status === 'completed' || status === 'succeeded') {
        const imageUrl = getFirstSub2ApiImageUrl(payload)
        if (!imageUrl) {
            return { status: 'failed', error: 'Xingtuo image completed but output URL missing' }
        }
        return {
            status: 'completed',
            resultUrl: imageUrl,
    ...
    if (status === 'failed' || status === 'error' || status === 'canceled' || status === 'cancelled') {
        const errorRaw = payload && typeof payload === 'object'
            ? (payload as { error?: unknown }).error
            : ''
        return {
            status: 'failed',
            error: typeof errorRaw === 'string' && errorRaw.trim()
                ? errorRaw.trim()
                : `Xingtuo image task failed: ${status || requestId}`,
        }
    }
...
function readStringFromRecords(records: Array<Record<string, unknown> | null>, keys: string[]): string {
...
function normalizeThirdpartyVeoStatus(rawStatus: string): 'pending' | 'completed' | 'failed' | 'unknown' {
    const status = rawStatus.trim().toLowerCase()
    if (!status) return 'unknown'
    if (status === 'queued' || status === 'in_progress' || status === 'processing' || status === 'in-progress') return 'pending'
    if (status === 'completed' || status === 'success' || status === 'succeeded') return 'completed'
    if (status === 'failed' || status === 'failure' || status === 'error' || status === 'canceled' || status === 'cancelled') return 'failed'
    return 'unknown'
}

async function pollThirdpartyVeoTask(taskId: string, userId: string): Promise<PollResult> {
    const config = await getProviderConfig(userId, THIRDPARTY_VEO_PROVIDER_ID)
    if (!config.baseUrl) {
        throw new Error(`PROVIDER_BASE_URL_MISSING: ${config.id}`)
    }

    const baseUrl = config.baseUrl.replace(/\/+$/, '')
    const response = await fetch(`${baseUrl}/videos/${encodeURIComponent(taskId)}`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${config.apiKey}` },
        cache: 'no-store',
    })
    ...
    if (!response.ok) {
        return {
            status: 'failed',
            error: `Thirdparty Veo status request failed: ${response.status} ${rawText.slice(0, 200)}`,
        }
    }
    ...
    const rawStatus = readStringFromRecords([data, root], ['status'])
    const status = normalizeThirdpartyVeoStatus(rawStatus)

    if (status === 'pending' || status === 'unknown') {
        return { status: 'pending' }
    }

    const outputUrl = readStringFromRecords([data, root], ['video_url', 'url', 'result_url'])
    if (status === 'completed') {
        if (!outputUrl) {
            return { status: 'failed', error: 'Thirdparty Veo completed but output URL missing' }
        }
        return {
            status: 'completed',
            videoUrl: outputUrl,
    ...
    const nestedError = getNestedRecord(data, 'error')
    const errorCode = readStringFromRecords([nestedError], ['code'])
    if (errorCode === 'extend_failed' && outputUrl) {
        return {
            status: 'completed',
            videoUrl: outputUrl,
            resultUrl: outputUrl,
    ...
    const errorMessage = readStringFromRecords([nestedError, data, root], [
        'message',
        'error',
        'fail_reason',
    ])
    return {
        status: 'failed',
        error: errorMessage || `Thirdparty Veo task failed: ${rawStatus || taskId}`,
    }
}
...
export function formatExternalId(
    provider: 'FAL' | 'ARK' | 'GEMINI' | 'GOOGLE' | 'MINIMAX' | 'VIDU' | 'OPENAI' | 'OCOMPAT' | 'BAILIAN' | 'SILICONFLOW' | 'SUB2API' | 'XINGTUO' | 'THIRDPARTY_VEO',
