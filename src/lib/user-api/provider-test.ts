...
type PresetProviderType = 'ark' | 'google' | 'openrouter' | 'minimax' | 'fal' | 'vidu'
  ...
  | 'siliconflow'
  | 'xingtuo'
type CompatibleProviderType = 'openai-compatible' | 'gemini-compatible' | 'sub2api' | 'xingtuo'
...
const DEFAULT_SUB2API_BASE_URL = 'http://121.127.253.220:8080'
const DEFAULT_XINGTUO_BASE_URL = 'http://121.127.253.220:8090/v1'
...
async function testXingtuoProvider(baseUrl: string | undefined, apiKey: string): Promise<TestProviderResult> {
  const url = `${(baseUrl || DEFAULT_XINGTUO_BASE_URL).trim().replace(/\/+$/, '')}/xingtuo/image2/jobs`
  try {
    const response = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    if (response.status === 400 || response.status === 422 || response.status === 202) {
      return {
        success: true,
        steps: [{
          name: 'imageGen',
          status: 'pass',
          message: 'Image endpoint authenticated',
...
export async function testProviderConnection(payload: TestProviderPayload): Promise<TestProviderResult> {
  ...
  switch (apiType) {
    ...
    case 'gemini-compatible':
      return testCompatibleProvider(baseUrl!, apiKey, llmModel)
    case 'sub2api':
      return testCompatibleProvider(baseUrl || DEFAULT_SUB2API_BASE_URL, apiKey, llmModel)
    case 'xingtuo':
      if (llmModel) {
        return testCompatibleProvider(baseUrl || DEFAULT_XINGTUO_BASE_URL, apiKey, llmModel)
      }
      return testXingtuoProvider(baseUrl, apiKey)
    case 'ark':
