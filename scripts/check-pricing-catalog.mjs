...
const INTERNAL_PRICING_CONDITION_FIELDS = {
...
function validateMediaCapabilityTierFields(issues, file, index, item, tiers, capabilityOptionFieldsMap) {
  ...
  const internalConditionFields = INTERNAL_PRICING_CONDITION_FIELDS[modelType] || new Set()
  ...
  for (let tierIndex = 0; tierIndex < tiers.length; tierIndex += 1) {
    ...
    for (const field of Object.keys(tier.when)) {
      if (!optionFields.has(field) && !internalConditionFields.has(field)) {
        pushIssue(
          issues,
          file,
          index,
